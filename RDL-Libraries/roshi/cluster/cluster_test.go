...
func TestInsertSelectOffsetKeys(t *testing.T) {
	// In order to start a fresh testing, uncomment the following line
	// Also, run redis-server in a different CLI
	os.Setenv("TEST_REDIS_ADDRESSES", "localhost:6379")
...
func TestExhInterleaving(t *testing.T) {
	os.Setenv("TEST_REDIS_ADDRESSES", "localhost:6379")
	addresses := os.Getenv("TEST_REDIS_ADDRESSES")
	if addresses == "" {
		t.Logf("To run this test, set the TEST_REDIS_ADDRESSES environment variable")
	...
	for i := 0; i < total_ils; i++ {
		// Build a new cluster with a high max size.
		r1 := integrationCluster(t, addresses, 1000)
		fmt.Println()
		fmt.Println("#### Start Interleaving: ", intls_count, " ####")
		file, err := os.Open("interleavings/ils_" + strconv.Itoa(intls_count) + ".txt")
		intls_count++
		...
		for scanner.Scan() {
			...
			switch operation {
			case "insert":
				r1.Insert([]common.KeyScoreMember{{key, score, member}})
			case "delete":
				r1.Delete([]common.KeyScoreMember{{key, score, member}})
			}
		}

		m1 := map[string][]common.KeyScoreMember{}

		for e := range r1.SelectOffset([]string{"foo", "bar"}, 0, 10) {
			if e.Error != nil {
				t.Errorf("during Select: key %q: %s", e.Key, e.Error)
			}
			m1[e.Key] = e.KeyScoreMembers
		}
		expected := []common.KeyScoreMember{
			{"foo", 1, "bar"},
			{"bar", 3, "rg"},
